...
export class LdSuiteLoader {
  ...
  getSignatureSuiteForKeyType(
    ...
    const suite =
      verificationType && this.signatureMap[verificationType]
        ? this.signatureMap[verificationType]
        : this.signatureMap[type];
  ...
  getAllSignatureSuiteTypes(): string[] {
    return Object.values(this.signatureMap).map((x) =>
      x.getSupportedVerificationType()
    );
  }
}
